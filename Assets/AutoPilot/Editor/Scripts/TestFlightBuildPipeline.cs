...
{	
	...
	public static bool CanBuild(TestFlightPreferences preferences, TestFlightMobileProvision[] availableProvisions, string[] availableIdentities)
	{
		...
		if(TestFlightBuildPipeline.IsTestFlightSDKInstalled() 
			&& preferences.userPrefs.testFlightSDKEmbedType == TestFlightSDKEmbedType.ForAppStoreSubmission 
			&& preferences.userPrefs.xCodeSchema.Length == 0)
			return false;
	...
	private static void ApplyTestflightSDKSettingToXcodeProject(TestFlightPreferences preferences)
	{
		...
		// 4.2
		string path = Path.GetFullPath(preferences.teamPrefs.buildPath)+"/Classes/UnityAppController.mm";
		if(!File.Exists(path)) 
		{
			// pre 4.2
			path = Path.GetFullPath(preferences.teamPrefs.buildPath)+"/Classes/AppController.mm";
		}
		...
		takeOffFunction += "\t[TestFlight takeOff:@\""+preferences.teamPrefs.appToken+"\"];\n"; 
...
//		TestFlightSDKSplashScreen.ShowSplashScreen();
	...
	public static bool BuildPlayerIOS (TestFlightPreferences preferences, bool logging=true)
	{
		PreBuildPlayer(preferences);
		
		bool buildResult = BuildPlayer(BuildTarget.iOS, preferences.teamPrefs.buildPath, logging);
	...
	public static bool BuildPlayer(BuildTarget target, string path, bool logging)
	{
		string errorStr = "";
		...
		{
			if(logging)	Debug.LogError("AutoPilot: Build failed, no scenes to build! Please add your scenes to Build Settings.");
			return false;
		}
		...
		*/	
		...
		{
			if(logging)	Debug.Log("AutoPilot: Existing xcode project found: appending");
		...
		{
			if(logging)	Debug.Log("AutoPilot: No existing xcode project: building clean");
	...
	{
		TestFlightXCodeSchemas schmeas = TestFlightXCodeSchemas.Enumerate(preferences);
		...
		*/
		...
		string stages = "clean build ";
		if(preferences.userPrefs.xCodeSchema.Length > 0 
			&& preferences.userPrefs.testFlightSDKEmbedType == TestFlightSDKEmbedType.ForAppStoreSubmission
			&& TestFlightBuildPipeline.IsTestFlightSDKInstalled())
			stages += " archive ";
		
		string args = stages+"-project Unity-iPhone.xcodeproj"+extraArgs;
