...
class TestFlightPreferencesWindow : TestFlightWindow
{
	...
	{		
		...
		tfpw.minSize = new Vector2(800, 720);
		tfpw.Show();
	}
	...
	public void OnEnable()
	{
		...
		TestFlightNonProSchemaWindow.OnSchemasLoaded += OnScemasReloaded;
		
		allProvisions = TestFlightMobileProvision.EnumerateProvisions();
		allSchemas = TestFlightXCodeSchemas.Enumerate(preferences);
		allIdentities = TestFlightDeveloperIdentities.Enumerate();
	}

	void OnScemasReloaded (TestFlightXCodeSchemas schema)
	{
		allSchemas = schema;
		if(preferences.userPrefs.xCodeSchema == "" && schema.Schemes.Length > 0)
			preferences.userPrefs.xCodeSchema = schema.Schemes[0];
	}
	...
	{	
		...
		{
			GUILayout.Box(new GUIContent("Save & Build", "You can't build & upload because your user or team API token hasn't been set"), GUILayout.Width(92), GUILayout.Height(30));
		}
	...
	private void OnGUI_TeamSettings()
	{
		...
		OnGUI_TeamSettings_TeamToken ();
		GUILayout.Space(10);
		OnGUI_TeamSettings_BuildPath ();
		GUILayout.Space(10);
		OnGUI_TeamSettings_DistLists();
		GUILayout.Space(10);
		OnGUI_TeamSettings_SDKInstall ();
		GUILayout.Space(10);
		OnGUI_TeamSettings_AppToken ();				
		
		EditorGUILayout.EndVertical(); 
		EditorGUILayout.EndHorizontal(); 
	}

	void OnGUI_TeamSettings_AppToken ()
	{
		preferences.teamPrefs.appToken = TextField(new GUIContent("App Token", "The testflight API token for this application"), preferences.teamPrefs.appToken).Replace(" ","");
		
		EditorGUILayout.BeginHorizontal(); 
		
		GUILayout.Space(FieldWidth);			
		
		if(GUILayout.Button("New App Token...", GUILayout.Width(180)))
		{
			Application.OpenURL("https://testflightapp.com/dashboard/applications/create/");
		}
		
		if(GUILayout.Button("Lookup Existing App Tokens...", GUILayout.Width(180)))
		{
			Application.OpenURL("https://testflightapp.com/dashboard/applications/");
		}
	
		if(preferences.teamPrefs.appToken.Length==0 && TestFlightBuildPipeline.IsTestFlightSDKInstalled ())
		{			
			Color baseColor = GUI.backgroundColor;
			GUI.backgroundColor = Color.red;
			GUILayout.Box("Please set your app token to use testflight SDK!", "textarea");
	...
	void OnGUI_TeamSettings_SDKInstall ()
	{
		GUILayout.BeginHorizontal();
		
		Label(new GUIContent("Testflight SDK"));
	...
	void OnGUI_TeamSettings_BuildPath ()
	{
		GUILayout.BeginHorizontal();
	...
	void OnGUI_TeamSettings_TeamToken ()
	{
	...
	private void OnGUI_UserSettings()
	{
		...
		DrawSubUserSettings(preferences, ref showUserLists, false, allProvisions, ref allSchemas, allIdentities);
				
		EditorGUILayout.EndVertical(); 
		EditorGUILayout.EndHorizontal(); 
	}
	
	public static void DrawSubUserSettingsForIPA(TestFlightPreferences preferences, bool narrow, TestFlightMobileProvision[] availableProvisions, ref TestFlightXCodeSchemas availableSchemas, string[] availableIdentities)
	{
		...
		DrawSubUserSettings_XCodeSchema(preferences, narrow, ref availableSchemas);
	...
	public static void DrawSubUserSettings(TestFlightPreferences preferences, ref bool showUserLists, bool narrow, TestFlightMobileProvision[] availableProvisions, ref TestFlightXCodeSchemas availableSchemas, string[] availableIdentities)
	{
		DrawSubUserSettings_TestFlightUpload(preferences, ref showUserLists);
		DrawSubUserSettingsForIPA(preferences, narrow, availableProvisions, ref availableSchemas,availableIdentities);
	}
	...
	static void DrawSubUserSettings_BundleId (TestFlightMobileProvision[] availableProvisions, string[] availableIdentities)
	{
		...
		int prevBundle = 0;
		prevBundle = Mathf.Max(0,bundleIdOptions.FindLastIndex(m => PlayerSettings.applicationIdentifier.StartsWith(m.Replace("*",""))));
		...
		if(bundleIdOptions[newChosenBundle].EndsWith("*"))
		{
			...
			if(PlayerSettings.applicationIdentifier.StartsWith(bundleIdOptions[prevBundle].Replace("*","")))
				appsuffix = PlayerSettings.applicationIdentifier.Remove(0, bundleIdOptions[prevBundle].Replace("*","").Length);
	...
	private static void DrawSubUserSettings_Provisions(TestFlightPreferences preferences, bool narrow, TestFlightMobileProvision[] availableProvisions, string[] availableIdentities)
	{
		...
		{	
			...
			{
				if(!provision.MatchesBundleId(PlayerSettings.applicationIdentifier))
					continue;
		...
		for(int i=0; i<identityProvisionOptions.Count; ++i)
		{
			...
			string output = option.Key+"/\n"
							+(option.Value.MatchesBundleId(PlayerSettings.applicationIdentifier)?"":"<Different Bundle Id> ")
							+option.Value.Name;
	...
	private static void DrawSubUserSettings_XCodeSchema(TestFlightPreferences preferences, bool narrow, ref TestFlightXCodeSchemas availableSchemas)
	{
...
//			defaultConfig = availableSchemas.DefaultConfig;
		...
		*/
		...
		if(GUILayout.Button(availableSchemas == null ? "Scan for schemes":"Rescan schemes", GUILayout.ExpandWidth(false)))
		{
			if(TestFlightBuildPipeline.HasPro())
			{	
				availableSchemas = TestFlightXCodeSchemas.Enumerate(preferences, true);
				schemes = availableSchemas.Schemes;
				newScheme = Mathf.Clamp(newScheme, 0, schemes.Length);
			}
			else 
			{
				TestFlightNonProSchemaWindow.ShowShemaWindow();
				//EditorUtility.DisplayDialog("Non-Pro schema detection", "To rebuild schemas in unity basic, just build your for iPhone", "Ok!");
			}
		}
		
		GUILayout.EndHorizontal();
		
		if(TestFlightBuildPipeline.IsTestFlightSDKInstalled() && preferences.userPrefs.testFlightSDKEmbedType == TestFlightSDKEmbedType.ForAppStoreSubmission)
		{
			if(availableSchemas == null)
				GUI.color = Color.red;
			else 
				GUI.color = Color.Lerp(Color.red, Color.yellow, 0.5f);
			
			GUILayout.Space(2);
			GUILayout.BeginHorizontal(GUILayout.ExpandWidth(false));
			GUILayout.Space(FieldWidth+4);
			GUILayout.Box("Building for Testflight Live requires a schema\nAutopilot will add this build to Xcode's organizer.");
			GUILayout.EndHorizontal();
			
			GUI.color = Color.white;
		}
		
		preferences.userPrefs.xCodeSchema = availableSchemas != null && newScheme < schemes.Length ? schemes[newScheme]:"";
	}
	...
	private static void Label(GUIContent label, int indent=0)
	{
		GUILayout.Space(indent);
		GUILayout.Label(label, GUILayout.Width(FieldWidth-indent));
	}
