...
//    private bool ex = false;
    ...
    // and try to convert it to match previous position but... that's not possible because Screen.width/Screen.height returns wrong window size.
    // Regardless of that I will still keep this that way
    protected void PropertySpecialNormalized(SerializedProperty coords, SerializedProperty normalized) {
        ...
        if (!normalizedBefore && normalized.boolValue) {
            coords.vector2Value = new Vector2(coords.vector2Value.x / t.storedScreenWidth, coords.vector2Value.y / t.storedScreenHeight);
