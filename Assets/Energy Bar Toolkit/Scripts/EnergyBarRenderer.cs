...
//    private Material radialMaterial; // material for radial bar
    ...
    protected override Rect TexturesRect {
        get {
...
#region OnGUI
    new public void OnGUI() {
        ...
        // workaround to prevent material leaks
...
//        Assert(labelSkin != null, "Label skin is not set!");
        var skin = labelSkin;
        if (skin == null) {
            skin = GUI.skin;
        }
        ...
        Rect rect = new Rect(screenPositionReal.x + labelPosition.x, screenPositionReal.y + labelPosition.y, size.x, size.y);
