...
public class csMissionBriefTitle : MonoBehaviour
{
		...
		{		
				if (grGlobals.m_playerLoggedIn) {			
						DoTextEffect (grGlobals.m_mBriefTitle, grGlobals.m_mBriefTitleColor);
				}
				//else if(!grGlobals.m_isConnectedtoInternet)
				//{
				//	DoTextEffect("Mission computer unavailable", Color.yellow);
				//}
		}
