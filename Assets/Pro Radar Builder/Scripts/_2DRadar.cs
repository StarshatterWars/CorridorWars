 ...
 RadarDesign.camera.transform.eulerAngles.y);
                ...
                if (!_RadarCenterObject2D.Instanced)
                {
                        ...
                        _RadarCenterObject2D._CreateBlipAs == CreateBlipAs.AsMesh)
                    {
                        ...
                        if (!RadarDesign.UseUI)
                        {
                            ...
                            _RadarCenterObject2D.CenterBlip.transform.SetParent(RadarDesign.BlipsParentObject.parent);
                ...
                if (item.IsActive)
                {
                    ...
                    for (var i = 0; i < trackedObjectCount; i++)
                    {
                        ...
                        if (distance < dynamicTrackingBOunds && distance >= dynamicInnerCullingZone)
                        {
                            ...
                            var currentBlipScale = item.BlipCanScleBasedOnDistance ? Mathf.Clamp(item.BlipMaxSize - (item.BlipMaxSize * (distance / dynamicTrackingBOunds)),
                                    item.BlipMinSize,
                        ...
                        if (distance < dynamicInnerCullingZone ||
                            distance > dynamicTrackingBOunds && !item.AlwaysShowBlipsInRadarSpace)
        ...
        private void Minimap()
        {
            ...
            if (minimapModule.mapType == MapType.Static)
            {
                ...
                minimapModule.Map.transform.localPosition = new Vector3(trackedObjectPos.x - distancefromMapCenter.x, trackedObjectPos.z - distancefromMapCenter.z, -0.01f);
